  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      if (query.length > 1 || query.length < 0) {
        return res.status(500).json({ msg: "Não foi possível achar o piloto" });
      }
      ...
      const data =
        await prismaClient.$queryRaw`SELECT nome_status, CAST(quantidade AS int) FROM lista_status(CAST('Piloto' AS TEXT), CAST(${idoriginal} AS INTEGER));`;
